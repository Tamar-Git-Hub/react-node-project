...
const LostDetails = () => {
  const { id } = useParams();
  const { data: lost, isLoading: isLostLoading, isError: isLostError } = useGetLostByIdQuery(id ? id : skipToken);
  ...
  return (
    <div>
      {isLostLoading || isOwnerLoading ? (
        <div>Loading...</div>
      ) : isLostError || isOwnerError ? (
        <div>error...</div>
      ) : lost ? (
        <div style={loginBox}>
          <div>
            ...
            <Typography sx={Alltext}>רחוב: {lost.street || ""}</Typography>
            <Typography sx={Alltext}>קטגוריה: {lost.category || ""}</Typography>
